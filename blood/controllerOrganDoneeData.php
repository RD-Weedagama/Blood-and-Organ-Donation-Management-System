...
if(isset($_POST['organdonor'])){
    ...
    $email_check = "SELECT * FROM organ_donee WHERE email = '$email'";
    ...
    if(count($errors) === 0){
        ...
        $insert_data = "INSERT INTO organ_donee (name, email, password, code, status,gender,nic,dob,address,contact_no,blood_group)
    ...
    if(isset($_POST['check'])){
        ...
        if(mysqli_num_rows($code_res) > 0){
            ...
            $code = 0;
            $status = 'verified';
            $update_otp = "UPDATE organ_donee SET code = $code, status = '$status' WHERE code = $fetch_code";
...
if(isset($_POST['save'])){
 ...
 if(isset($_POST['personal'])){
    ...
    if(mysqli_connect_error()){
        ...
        $update_don = "UPDATE organ_donee SET gender = '$gender', nic = '$nic', dob = '$dob', address = '$address', contact_no = '$contact_no' WHERE email = '$email'";
        $update_res = mysqli_query($con, $update_don);
             	// Get name of images
  	$Get_image_name = $_FILES['image']['name'];
...
if(isset($_POST['save'])){
    ...
    if(mysqli_connect_error()){
        ...
        $update_don = "UPDATE organ_donee SET name = '$a', email = '$b', gender = '$c', nic = '$d', dob = '$e', address = '$f', contact_no = '$g'  WHERE email = '$email'";
        $update_res = mysqli_query($con, $update_don);
        if($update_res){
    
             header('Location: organ-donee-profile.php');
        }else{
        $errors['db-error'] = "Failed!";
            }
